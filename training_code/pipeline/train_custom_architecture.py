...
import pandas as pd
import time
from new_architecture import generate_ncp_model
...
# options = tf.data.Options()
# options.experimental_distribute.auto_shard_policy = tf.data.experimental.AutoShardPolicy.DATA
# training_dataset = dataset.with_options(options)
# validation_dataset = validation_dataset.with_options(options)
# # Have GPU prefetch next training batch while first one runs
# training_dataset = training_dataset.prefetch(tf.data.AUTOTUNE)
# # validation_dataset = validation_dataset.prefetch(tf.data.AUTOTUNE)
...
# epochs: int = 1
# csv_logger = tf.keras.callbacks.CSVLogger('training_log_new_diff_scratch_wscheduler0.85_seed22222_lr{lr}_trainloss{train_loss:.5f}_epoch{epochs}.csv', separator=',', append=False)
# callbacks = None
