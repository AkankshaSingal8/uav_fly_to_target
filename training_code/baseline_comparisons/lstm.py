...
with strategy.scope():
    mymodel = generate_lstm_model(rnn_sizes, seq_len, IMAGE_SHAPE)
    mymodel.compile(optimizer=optimizer, loss="mean_squared_error", metrics=['mse'])
    mymodel.load_weights('./baseline_comparisons/model-lstm_seq-64_lr-0.000290_epoch-085_val-loss_0.0711_train-loss_0.0023_mse_0.0023_2022_04_15_05_11_33.hdf5')
...
# epochs: int = 100
# csv_logger = tf.keras.callbacks.CSVLogger(f'./baseline_comparisons/lstm_mix_goal_{decay_rate}_seed22222_lr{lr}_epoch{epochs}.csv', separator=',', append=False)
# callbacks = None
...
# mymodel.save(f'./baseline_comparisons/lstm_mix_goal_{decay_rate}_seed22222_lr{lr}_trainloss{train_loss:.5f}_epoch{epochs}.h5')
